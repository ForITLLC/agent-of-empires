...
async fn run_detached_restart(state: Arc<AppState>, id: String) {
    ...
    let restart_result = tokio::task::spawn_blocking(move || {
        crate::session::restart::perform_restart(crate::session::restart::RestartRequest {
            ...
            size: None,
            wake_message,
            // Same shape as the TUI's user-initiated restart: run on_launch
            // hooks, bound a hung hook so it cannot wedge the daemon worker,
            // keep the sandbox container (no tool swap on this path).
